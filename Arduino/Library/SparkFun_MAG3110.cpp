...
bool MAG3110::initialize(uint8_t scl, uint8_t sda)
{
	//Serial.println("USING HIGHE I2C");
	SWire.begin(scl, sda);
	SWire.setClock(400000); // I2C fast mode, 400kHz
...
uint8_t MAG3110::readRegister(uint8_t address)
{
	...
	SWire.beginTransmission(MAG3110_I2C_ADDRESS);
	SWire.write(address);
	SWire.endTransmission();

	delayMicroseconds(2);

	SWire.requestFrom(MAG3110_I2C_ADDRESS, 1);
...
void MAG3110::readMag(int *x, int *y, int *z)
{
	...
	SWire.beginTransmission(MAG3110_I2C_ADDRESS);
	SWire.write(MAG3110_OUT_X_MSB);
	SWire.endTransmission();

	delayMicroseconds(2);

	// Read out data using multiple byte read mode
	SWire.requestFrom(MAG3110_I2C_ADDRESS, 6);
	...
	{
		values[idx] = SWire.read() << 8; // MSB
		values[idx] |= SWire.read();	 // LSB
	}
